        ...
        if (
            ...
            try {
                ...
                foreach ($archivos['tmp_name'] as $indice => $tmp_name) {
                    ...
                    if (!$subir) {
                        throw new Exception("Failed to move file from $tmp_name to $ruta_destino");
                    }
    
                    // Insert record into database
                    $consulta->bind_param("si", $nombre_archivo, $categoria);
                    $consulta->execute();
                }
    ...
    public function eliminarFotosSeleccionadas($seleccionadas, $categoria)
    {
        ...
        try{
            foreach ($seleccionadas as $idFoto) {
                ...
                $url = $fila['url'];
                $directorio= dirname(__DIR__) . "../../assets/images/categorias/" . $nombreCategoria . "/" . $nombre_archivo;
